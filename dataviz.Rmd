...
* Then, we move on the much more powerful **ggplot2 package** (part of the tidyverse). 
  ...
  + step by step, we will replicate the graph above (but with some lengthy digressions to learn about different ggplot options inbetween. 
...
Base R has several functions for data visualization. Crucially, you need sepearte functions for each type of plot. Some examples:
...
* Use the `geom_bar()` function.
  ...
  + use stat="identity" to indicate that the height of the bars should represent values in the data (default is stat = "bin" which simply counts the number of occurences of the x aestehetic)
...
#### Playing with thems and other stuff
...
Some random ideas for the theme:
...
Jsut to mention this, you can save a ggplot object in the environment like everything else (remember, everything in R is an object).
...
We can then add stuff to the the ggplot object in the usual way.
...
[Book](http://socviz.co/index.html#preface) on Data Visualizaton by Kieran Healy, freely available online