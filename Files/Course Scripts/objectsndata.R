...
#' representing the variables. As such, it is similar to a matrix. For us, the most important difference to a matrix is that a data frame can contain variables of different
#' types (like numerical, logical, string and factor), whereas matrices can only contain numerical values. 
#' 
#' Unlike a matrix, the columns always contain names which represent the variables. We can define a data frame from scratch by using the command ```data.frame``` or ```as. data.frame```
#' 
#' <span style="color:red">Task</span> <u>**Complete the empty spaces!**</u>
## ---- eval=FALSE---------------------------------------------------------
...
#' 
#' The outputs of matrix ```presence.mat``` and ```presence``` look exactly the same, but they behave differently. In RStudio, the difference can be seen in the Workspace window. ```presence``` is described as *5 obs. of 3 variables*. Again, every column is an individual variable. 
#' 
