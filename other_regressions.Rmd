...
However, it is a best-practice to look for missing values in your data prior to the estimation to keep your code transparent.
...
- `residuals`
- `fitted.values` The estimates for our dpendent variable (unemployment)
...
Jake Russ has the [ultimative overview](https://www.jakeruss.com/cheatsheets/stargazer/) over all `stargazer()` functions (there are many!!).
...
Install the plm package first. 

The basic structure for a two-way FE panel regression is as follows: 
...
```{r}
p1 <-
  plm(unemp_workagepop_t ~ gdp_gr + inv_per_empl + immigration_t,
      index=c("geo_code", "time"), model="within", effect= "twoways",
      data = eur_data)
...
Within plm, you can easily switch between different econometric models (one-way FE, random effects, pooling, etc.).
